...
class CodeSearcher:
    ...
    def load_use_data(self):
         ...
         logger.debug('sim_desc')
         chunk_sim_descs=pickle.load(open(self.path+self.data_params['valid_sim_desc'], 'rb'))
         return chunk_tokens[:],chunk_names[:],chunk_apis[:],chunk_sbts[:],chunk_sim_descs[:]
    ...
    def train(self, model):
        ...
        for i in range(self.train_params['reload']+1, nb_epoch):
            ...
            print("tokens:",len(chunk_tokens),len(chunk_tokens[0]))
            print("sim_descs:", len(chunk_sim_desc), len(chunk_sim_desc[0]))
            print("descs:", len(chunk_descs), len(chunk_descs[0]))
            print("asts:", len(chunk_sbts), len(chunk_sbts[0]))
            ...
            print(f"ast:{chunk_padded_sbts.shape}")
            print("chunk_padded_tokens:",type(chunk_padded_tokens))
            ...
            # index_token = list(range(0, len(chunk_tokens)))
            random.shuffle(index)
            # random.shuffle(index_token)
            ...
            # chunk_sim_tokens_bad=[chunk_sim_tokens[id] for id in index]
            ...
            # chunk_padded_sim_tokens_bad=self.pad(chunk_sim_tokens_bad, self.data_params['sim_tokens_len'])
    ...
    def eval(self, model, K):
        ...
        for i in tqdm(range(data_len)):
            ...
            n_results = K
            sims = model.predict([tokens, names, apis, sbts, sim_desc, descs], batch_size=1000).flatten()
            # sims = self.Whitening(token_reprs, desc_reprs).flatten()
            negsims=np.negative(sims)
            predict_origin=np.argsort(negsims)#predict = np.argpartition(negsims, kth=n_results-1)
            predict = predict_origin[:n_results]   
            predict = [int(k) for k in predict]
    ...
    def search(self,model,query,n_results=10):
        ...
        return codes_out, sims_out  # paired codes and their similarities
...
if __name__ == '__main__':
    K.set_session(get_session(0.90))  # using 80% of total GPU Memory
    args = parse_args()
    ...
    #     vecs=codesearcher.repr_code(model)
        ...
        queriesFile = codecs.open('./results/query_sw.txt', "r", encoding='utf-8', errors='ignore')#removed stopwords for query
        n_results = 10
